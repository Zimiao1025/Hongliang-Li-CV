...
\usepackage[a4paper,top=0.48in,bottom=0.48in,left=0.55in,right=0.55in]{geometry}
\usepackage{fontspec}
...
\newcommand{\entry}[4]{%
  ...
  \end{tabularx}\par\vspace{0.04em}
}

\newenvironment{detailitems}{%
  \begin{itemize}[leftmargin=2.15em,label=$\circ$,itemsep=0.08em,topsep=0.10em,parsep=0pt,partopsep=0pt]
}{\end{itemize}}

\newcommand{\project}[2]{%
  \noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}Xr@{}}
    \textbullet\enspace\textbf{#1} & #2
  \end{tabularx}\par\vspace{0.02em}
}

\newcommand{\publication}[1]{%
  \begin{itemize}[leftmargin=1.18em,label=\textbullet,itemsep=0.20em,topsep=0.08em,parsep=0pt,partopsep=0pt]
    \item #1
  \end{itemize}
}
...
\begin{center}
  {\fontsize{24.8}{27}\selectfont\bfseries \CVName}\par
  \vspace{0.20em}
...
\begin{detailitems}
  \item AI-powered scientific workbench integrating machine-learning models, molecular modeling tools, and multi-omics data to support rational drug design and hit-to-lead optimization.
\end{detailitems}

\project{AIRFold}{\href{https://atomlab.yanyanlan.com/project/airfold/}{Homepage}\enspace |\enspace \href{https://github.com/THU-ATOM/AIRFold}{GitHub}}
\begin{detailitems}
  \item Protein structure prediction system that achieved top-ranking performance in CAMEO.
\end{detailitems}

\section{Publications}
{\bfseries\itshape Submitted / Under Review}
\publication{\textbf{Hongliang Li} et al. (2026). ``OriScore: Unified Affinity Scoring Framework for All Drug Modalities.'' \textit{Nature} (under review).}
